#!/usr/bin/env python
#
# Use the raw transactions API to spend 3dcoins received on particular addresses,
...
#
# Assumes it will talk to a 3dcoind or 3DCoin-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def read_bitcoin_config(dbdir):
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "3dcoin.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a 3DCoin Core JSON-RPC server"""
    testnet = config.get('testnet', '0')
    testnet = (int(testnet) > 0)  # 0/1 in config file, convert to True/False
    if not 'rpcport' in config:
        config['rpcport'] = 16015 if testnet else 15015
    connect = "http://%s:%s@127.0.0.1:%s"%(config['rpcuser'], config['rpcpassword'], config['rpcport'])
    try:
        result = ServiceProxy(connect)
        # ServiceProxy is lazy-connect, so send an RPC command mostly to catch connection errors,
        # but also make sure the 3dcoind we're talking to is/isn't testnet:
...
def list_available(D3dcoind):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-3dcoin-address
...
def main():
    ...
    parser.add_option("--from", dest="fromaddresses", default=None,
                      help="addresses to get 3dcoins from")
    parser.add_option("--to", dest="to", default=None,
                      help="address to get send 3dcoins to")
    parser.add_option("--amount", dest="amount", default=None,
                      help="amount to send")
    parser.add_option("--fee", dest="fee", default="0.0",
                      help="fee to include")
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of 3dcoin.conf file with RPC username/password (default: %default)")
